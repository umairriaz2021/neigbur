   ...
   <div class="outer-wrapper ">
      <div class="container container-home">
         ...
         <div class="event-detail">
           <img src="<?php $eve_img="https://storage.googleapis.com/".$event_detail->files[0]->bucket."/".$event_detail->files[0]->filename;?>" alt="">
...
HTML;
			...
			<?php $sendUrl =  site_url().'/get-tickets/'.$event_id;?>
             ...
             /*foreach($event_detail->tickets as $key=>$tkt){ 
               // var_dump($tkt->ticket_type);
                 foreach($tkt->ticket_type as $key=>$value){ 
                     
              $remainingtick =$value->max - $value->ticket_allocation;
              //echo  'remaining tickets' . $remainingtick;
             if ( $remainingtick == 0) { 
               // if (count($event_detail->ticketTypes) == 1) {
                 // if ((strtotime($event_detail->ticketTypes[0]->start) >= strtotime(date('Y-m-d'))) && (strtotime($event_detail->ticketTypes[0]->end) <= strtotime(date('Y-m-d')))) {
                ?>
                    <a href="<?php echo site_url() ?>/get-tickets/<?php echo $event_id; ?>"><button class="buy-ticket" type="button" name="btnSubmit">SOLD OUT</button></a>
                    <?php
                  //  }
                    //}
             }
  ...
  Launch demo modal
               ...
               <h3><?php echo isset($event_detail) ? $event_detail->name : ''; ?>
               
               <img class="topbtns  a2a_dd" 
               id="share_btn"
               <?php if($event_detail->ticketTypes[0]->price > 0 || $event_detail->ticketTypes[0]->price == 0 ): ?>
               style="right:0 !important";
               <?php endif;?> 
               href="https://www.addtoany.com/share" 
               src="<?php echo site_url(); ?>/wp-content/themes/Divi Child/img/sharepng.png" alt="uplaod images">
          ...
          <?php if($thirdparty):?>
					<a href="<?php if(!empty($thirdparty)){ echo 'https://'. $thirdparty;} ?>"><button class="buy-ticket" style="right:-100px;" type="button" name="btnSubmit">DETAILS</button>
						  </a>
					    <?php elseif(!empty($event_detail->ticketTypes) && $event_detail->ticketTypes[0]->price > 0 || (!empty($event_detail->ticketTypes) && $event_detail->ticketTypes[0]->price == 0)):?>
					    
                        <?php /*  foreach($event_detail->tickets as $key=>$tkt){ 
                 //var_dump($tkt->ticket_type);
                 //($tkt->ticket_type as $key=>$value){ 
                     
              $remainingtick = ($tkt->ticket_type->max) - ($tkt->ticket_type->ticket_allocation);
              //echo  'remaining tickets' . $value->ticket_allocation;
            
           //}
             }*/
              $cart = array();
              foreach($event_detail->ticketTypes as $key=>$tkt){
                  $max = $tkt->max; 
                  $sum = 0;
                
                 foreach($tkt->ticket_order_item as $key=>$value){
                     $a = $value->quantity;
                     $sum+= $a;
                 }
                  //echo $sum;
               ...
               if($tkt->ticket_order_item){
                  if ($sum == $max ){
                      // echo 'soldout';
                       ?>
                       <a href="<?php echo site_url()?>/get-tickets/<?php echo $event_id;?>/?tid=<?php if(($event_detail->ticketTypes) == 1 || ($event_detail->event_dates) > 1)  {  echo $event_detail->ticketTypes[0]->id; }?>&abc=<?php if(($event_detail->ticketTypes) == 1 || ($event_detail->event_dates) > 1) { echo $edate->start_date; }?>"><button class="event_btn"  type="button" name="btnSubmit" value="buybtn">SOLD OUT</button></a>
                  <?php }
                   /*else{
                       echo 'gettickets';
                       ?>
                 ...
                 //echo 'gettickets';
                 ?>
                 <a href="<?php echo site_url()?>/get-tickets/<?php echo $event_id;?>/?tid=<?php if(($event_detail->ticketTypes) == 1 || ($event_detail->event_dates) > 1)  {  echo $event_detail->ticketTypes[0]->id; }?>&abc=<?php if(($event_detail->ticketTypes) == 1 || ($event_detail->event_dates) > 1) { echo $edate->start_date; }?>"><button class="event_btn"  type="button" name="btnSubmit" value="buybtn">GET TICKETS</button></a>
                 <?php
             }
       ...
       <?php   }*/
             ...
             ?>
              ...
              <?php /* elseif(!empty($event_detail->ticketTypes) && $event_detail->ticketTypes[0]->price == 0): ?>
					    <a href="<?php echo site_url()?>/get-tickets/<?php echo $event_id;?>/?tid=<?php if(($event_detail->ticketTypes) == 1 || ($event_detail->event_dates) > 1) { echo $event_detail->ticketTypes[0]->id; } ?>&abc=<?php if(($event_detail->ticketTypes) == 1 || ($event_detail->event_dates) > 1) { echo $edate->start_date; }?>"><button class="event_btn"  type="button" name="btnSubmit" value="freebtn">GET TICKETS</button></a>
