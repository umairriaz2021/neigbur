                ...
                for (i = 0; i < filesAmount; i++) {
                    ...
                    reader.onload = function(event) {
                        if (pdfFileType.indexOf(extension) <= -1) {
                            ...
                            jQuery(input).siblings("#file_err").hide();
                            jQuery(input).siblings("#file_type_err").hide();
                            jQuery(input).siblings("#file_name").children("p").html(input.files[0].name);
                            jQuery(input).siblings("#file_name").show();
                            jQuery("#DDText1").hide();
                            jQuery("#pdfdropZone").css({"background-image": "url('https://webdev.snapd.com/wp-content/uploads/2019/09/pdf_icon.png')","border": "2px dashed #ffffff"});
                    ...
                    reader.onload = function(event) {
                        if (fileTypes.indexOf(extension) <= -1) {
                            ...
                            if(placeToInsertImagePreview == '#logo_files'){
								...
								jQuery(jQuery.parseHTML('<img class="uploaded-img">')).attr('src', event.target.result).appendTo(placeToInsertImagePreview);
								jQuery(jQuery.parseHTML('<span class="remove-img" style="cursor: pointer;">-</span>')).appendTo(placeToInsertImagePreview);
...
function HighlightArea(id, isTrue) {
	console.log(id);
	isTrue == true ? jQuery('#'+id).addClass('HighlightArea'):jQuery('#'+id).removeClass('HighlightArea');
}
...
function CropClick(previewDiv){
	...
	if(cropper){
		...
		if(previewDiv == '#files'){
			...
			jQuery(jQuery.parseHTML('<img class="uploaded-img">')).attr("src",canvas.toDataURL()).appendTo('#files');
			jQuery(jQuery.parseHTML('<span class="remove-img" style="cursor: pointer;">-</span>')).appendTo('#files');
...
function initAutocomplete() {
  ...
  document.getElementById('streetaddress2'), {types: ['geocode']});
  ...
  for (var i = 0; i < place.address_components.length; i++) {
    ...
    if (componentForm[addressType]) {
	  ...
	  if(addressType=='administrative_area_level_1'){
		...
		$('#state option').filter(function() { return $.trim( $(this).text() ) == value; }).prop("selected", true).attr('selected',true);
	  ...
	  if(addressType=='country')
    {
      console.log(value);
		  ...
		  $('#country option').filter(function() { return $.trim( $(this).text() ) == value; }).prop("selected", true).attr('selected',true);
