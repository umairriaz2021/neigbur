		...
		var AddFile = function(uploads, e, data) {
			...
			var StartUpload = function(e) {
				...
				if (settings.edit && !data.ff_info.errors.length)
				{
					...
					if (fileinput.length)
					{
						var newfilecaption = fileinput.val();
						var newfilename = fileinput.val();
						if (fileext != '')  newfilename += '.' + fileext;

						inforow.find('.ff_fileupload_filename').text(newfilecaption);
						data.files[0].uploadName = newfilecaption + '.' + fileext;
					}
				}
			...
			if (settings.edit && !data.ff_info.errors.length)
			{
				inforow.find('.ff_fileupload_filename').append($('<input placeholder="Enter caption">').attr('type', 'text').val(filename).keydown(function(e) {
			...
			inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize + (hasimage && settings.edit && !data.ff_info.errors.length ? ' | '+filename+'.' +fileext : ''));
