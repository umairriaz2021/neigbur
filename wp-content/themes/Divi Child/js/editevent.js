  ...
  function (value, element) {
    if ($("#country").val() == "2") {
      return (
        ...
        /^([A-Z][0-9][A-Z])\s*([0-9][A-Z][0-9])$/.test(value)
      );
    } else {
      return true;
    }
          ...
          reader.onload = function (event) {
            if (pdfFileType.indexOf(extension) <= -1) {
              ...
              $("#pdfdropZone").css(
                ...
                "url('" +
                  site_url +
                  "/wp-content/uploads/2019/09/upload-image.jpg')"
              );
              $("#dd_text_pdf").show();
              $("#btnpdf").show();
              ...
              $("#pdfdropZone").css(
                ...
                "url('" +
                  site_url +
                  "/wp-content/uploads/2019/09/pdf_icon.png')"
              );
              jQuery(
                jQuery.parseHTML(
                  '<span class="remove-img" id="remove-pdf" style="cursor: pointer;">-</span>'
                )
          ...
          reader.onload = function (event) {
            if (fileTypes.indexOf(extension) <= -1) {
              ...
              if (placeToInsertImagePreview == "#logo_files") {
                ...
                $($.parseHTML('<img class="uploaded-img" onClick="clickit()">'))
                  .attr("src", event.target.result)
                  .appendTo(placeToInsertImagePreview);
                jQuery(
                  jQuery.parseHTML(
                    '<span class="remove-img" id="removelogo" style="cursor: pointer;">-</span>'
                  )
                ).appendTo(placeToInsertImagePreview);
...
function CropClick(previewDiv) {
  ...
  if (cropper) {
    ...
    if (previewDiv == "#files") {
      ...
      jQuery(jQuery.parseHTML('<img id="eve_image" class="uploaded-img">'))
        .attr("src", canvas.toDataURL())
        .appendTo("#fileshide");
      jQuery(
        jQuery.parseHTML(
          '<span class="remove-img" id="removeimg" style="cursor:pointer;">-</span>'
        )
      ).appendTo("#files");
      jQuery(
        jQuery.parseHTML(
      ...
      $("#removeimg").on("click", function () {
        ...
        $("#dropZone").css(
          "background-image",
          "url('" + site_url + "/wp-content/uploads/2019/09/upload-image.jpg')"
        );
      });
...
jQuery(document).ready(function (e) {
  ...
  let eventid =
    "<?php echo isset($events) ? base64_encode($events->id) : '';?>";
...
jQuery("#event_form").submit(function (event) {
  if ($("#event_form").valid()) {
    jQuery("#edit_modal_loader_text").text(
      "Updating event details and images..."
    );
...
function pdfDelete() {
  ...
  empty_pdf_zone();
  setTimeout(function () {
    $("#loadingeditModal").css({ display: "none" });
  }, 2000);
}
function logoDelete() {
...
function imgDelete() {
  ...
  jQuery("#dropZone").css({
    "background-image":
      "url(http://webdev.snapd.com/wp-content/uploads/2019/09/upload-image.jpg)",
  });
}
